...
for li = 1:lmax
    ...
    for mi = -li:li
        ...
        if mi > 0
            conversion = (-1)^(abs(mi))*factorial(li-abs(mi))/factorial(li+abs(mi));
            p_limi = P_lm{li+1,abs(mi)+1}*conversion;
            pi_limi = Pi_lm{li+1,abs(mi)+1}*conversion;
            tau_limi = Tau_lm{li+1,abs(mi)+1}*conversion;
        else
        ...
        for lp = 1:lmax
            ...
            for mp = -lp:lp
                ...
                if mp > 0
                    ...
                    conversion = (-1)^abs(mp)*factorial(lp-abs(mp))/factorial(lp+abs(mp));
                    p_lpmp = P_lm{lp+1,abs(mp)+1}*conversion;
                    pi_lpmp = Pi_lm{lp+1,abs(mp)+1}*conversion;
                    tau_lpmp = Tau_lm{lp+1,abs(mp)+1}*conversion;
                end
                ...
                if selection_rules_1122 ~= 0
                    ...
                    J11(ni,np) = sum(sum(1i*weight_map.*r.^2.*prefactor.*j_lp.*b_li.*(p_lpmp*mp.*tau_limi+p_limi*mi.*tau_lpmp)));
                    ...
                    term1 =  1i*dj_lp.*db_li/ks/k.*(mi*tau_lpmp.*p_limi+mp.*tau_limi.*p_lpmp);
                    term2 = -1i*r.^2/ks/k.*ellip1.*st.*ct.*p_limi.*p_lpmp.*(mp*li*(li+1)*dj_lp.*b_li+mi*lp*(lp+1)*j_lp.*db_li);
                    term3 = -r.^2/k/ks.*ellip2.*sp.*cp.*st.^2.*(lp*(lp+1)*j_lp.*p_lpmp.*db_li.*tau_limi-li*(li+1)*dj_lp.*tau_lpmp.*b_li.*p_limi);
                ...
                if selection_rules_1221 ~= 0
                    ...
                    term1 = (-mi*mp.*pi_lpmp.*p_limi-tau_lpmp.*tau_limi).*r/k.*db_li.*j_lp;
                    term2 = r.^3/k*li*(li+1).*st.*b_li.*p_limi.*j_lp.*(ellip1.*st.*ct.*tau_lpmp+ellip2.*sp.*cp.*p_lpmp*1i*mp);
                    J12(ni,np) = sum(sum(weight_map.*prefactor.*(term1+term2)));
                    term1 = (-mi*mp.*pi_lpmp.*p_limi-tau_lpmp.*tau_limi).*r/k.*dj_lp.*b_li;
                    term2 = r.^3/ks*lp*(lp+1).*st.*b_li.*p_lpmp.*b_li.*(ellip1.*st.*ct.*tau_limi-ellip2.*sp.*cp.*p_limi*1i*mi);
