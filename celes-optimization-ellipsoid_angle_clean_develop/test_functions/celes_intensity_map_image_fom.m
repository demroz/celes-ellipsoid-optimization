...
particle1 = linspace(900,1000,numParts);
particle2 = linspace(900,1000,numParts)';

xl = linspace(-5000,5000,100);
yl = xl.';
...
inversePreconditioner.partitionEdgeSizes = [30000,30000,3000];
% for blockdiagonal preconditioner: edge size of partitioning cuboids
preconditioner.partitionEdgeSizes = [12000,12000,1200];
...
for i = 1:numParts^2
    ...
    E_t = compute_scattered_field_opt(simulation,points);%+compute_initial_field_opt(simulation,points);
%     intensities(i) = sum(abs(gather(E_t))).^2;
    intensities(i) = sum((image(:)-sum(abs(gather(E_t)).^2,2)).^2);
...
% %     timingdum(i) = toc;
    tic;
%     simulation=simulation.computeAdjointFields(E_t,point);
    simulation = simulation.computeImageAdjointFields(image,E_t,points);
...
hold on
quiver(radius_map1,radius_map2,dx(:),dy(:),'r');
axis square
